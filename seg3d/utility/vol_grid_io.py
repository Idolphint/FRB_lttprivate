...
def get_bounding_box_loc(img_path=None, img=None, bbx_ext=10):
    # print '### loading sample from:\n %s' % (im_folder+ '/' + str(idx))
    if img_path is not None:
        tempmat=loadmat(img_path)['data']
        tempmat=np.swapaxes(tempmat,0,2)   #返回结果为384*384*channel，长*宽*高
    else:
        tempmat = img
    #tempmat=tempmat['data'] #返回结果为channel*384*384
    vol_data = np.asarray(tempmat)  
    ...
    vol_mask = vol_mask.astype('int')
    # locate object in the volume
    loc = ndimage.find_objects(vol_mask)
    if DEBUG:
        print("in get_bounding_box_loc, find_objects loc: ",loc)
    loc = loc[0]
    # extend the bounding box
    bbx_loc = np.array([[loc[0].start, loc[1].start, loc[2].start], [loc[0].stop, loc[1].stop, loc[2].stop]])
    bbx_loc[0, :] = bbx_loc[0, :] - bbx_ext
    ...
    if bbx_loc[1, 2] > orig_dim[2]: bbx_loc[1, 2] = orig_dim[2]
    if DEBUG:
        print("bbx_ext, bbx_loc: ",bbx_ext, bbx_loc)
...
#main funtcion
def load_nii2grid(grid_D, grid_ita, img_path=None, img=None, bbx_loc=None):
    if img_path is not None:
        matfile=loadmat(img_path)['data']
        matfile = np.swapaxes(matfile, 0, 2)  # 返回结果为384*384*channel，长*宽*高
    else:
        matfile = img
    vol_data = np.asarray(matfile)  # 这行不知道干什么，先保留
    if DEBUG:
        print("check mat in load_nii2grid shape: ",vol_data.shape)
    # crop the volume according to bounding box
    if DEBUG:
        print("bbx_loc of bounding_box is: ",bbx_loc, bbx_loc.shape)
    if bbx_loc is not None: #bbx_loc是get_bounding_box 得到包含物体的box
        vol_data = vol_data[bbx_loc[0, 0]:bbx_loc[1, 0], bbx_loc[0, 1]:bbx_loc[1, 1], bbx_loc[0, 2]:bbx_loc[1, 2]]
...
def partition_vol2grid2seq(vol_data, cube_D, ita, norm_fact):
    ...
    if DEBUG:
        print("cala_data_scale: seq_r, seq_c, ovlap, VOL_data.shape", seq_r, seq_c, ovlap, vol_data.shape)
    # serialize image
    ...
    dim = np.asarray(vol_data.shape)
    if DEBUG:
        print("before grid, shape ",vol_data.shape)
    p_count = 0
    for R in range(0, fold[0]):
...
def deserialize_cubearray2grid(cube_array, grid_dim, cube_D, cube_ita):
    ...
    # reconstruct volume from grids
    print(grid_dim)
    label_0_mat = (np.zeros(grid_dim)).astype('float32')
    label_0_mat=torch.from_numpy(label_0_mat)
    #label_1_mat = (np.zeros(grid_dim)).astype('float32')
    #label_1_mat = torch.from_numpy(label_1_mat)
    #label_2_mat = (np.zeros(grid_dim)).astype('float32')
    #label_2_mat = torch.from_numpy(label_2_mat)
    #label_3_mat = (np.zeros(grid_dim)).astype('float32')
    #label_3_mat = torch.from_numpy(label_3_mat)
    add_count=(np.zeros(grid_dim)).astype('int')
    add_count=torch.from_numpy(add_count)
        ...
        for C in range(0, fold[1]):
            ...
            for H in range(0, fold[2]):
                ...
                if h_e >= grid_dim[2]:
                    ...
                    h_e = h_s + cube_D
                # histogram for voting
                idx_0 = (cube_array[p_count] )
                # idx_0 = (np.asarray(idx_0)).astype('int')
                #idx_1 = (cube_array[p_count] )
                # idx_1 = (np.asarray(idx_1)).astype('int')
                #idx_2 = (cube_array[p_count] )
                # idx_2 = (np.asarray(idx_2)).astype('int')
                #idx_3 = (cube_array[p_count] )
                # idx_3 = (np.asarray(idx_3)).astype('int')

                label_0_mat[r_s:r_e, c_s:c_e, h_s:h_e] = label_0_mat[r_s:r_e, c_s:c_e, h_s:h_e] + idx_0
                add_count[r_s:r_e, c_s:c_e, h_s:h_e]+=1
                #label_1_mat[r_s:r_e, c_s:c_e, h_s:h_e] = label_1_mat[r_s:r_e, c_s:c_e, h_s:h_e] + idx_1
                #label_2_mat[r_s:r_e, c_s:c_e, h_s:h_e] = label_2_mat[r_s:r_e, c_s:c_e, h_s:h_e] + idx_2
                #label_3_mat[r_s:r_e, c_s:c_e, h_s:h_e] = label_3_mat[r_s:r_e, c_s:c_e, h_s:h_e] + idx_3
    ...
    # print np.unique(label_mat)
    #print(add_count[0:7,0:7,0:12])
    return label_0_mat/add_count


def deserialize_gridarray2vol(grid_list, vol_dim, bbx_loc, grid_D, grid_ita):
    # create final volume
    desr_vol = (np.zeros(vol_dim)).astype('float32')
    desr_vol=torch.from_numpy(desr_vol)
    # create bounding box volume
    bbx_dim = np.array([bbx_loc[1, 0]-bbx_loc[0, 0], bbx_loc[1, 1]-bbx_loc[0, 1], bbx_loc[1, 2]-bbx_loc[0, 2]])
    bbx_dim=torch.from_numpy(bbx_dim)
    # pre-calculate the dataset scale
    seq_r, seq_c, fold, ovlap = calc_data_scale(bbx_dim, grid_D, grid_ita)
    # reconstruct volume from grids
    label_0_mat = (np.zeros((bbx_dim[0],bbx_dim[1],bbx_dim[2]))).astype('float32')
    label_0_mat=torch.from_numpy(label_0_mat)
    add_count=(np.zeros((bbx_dim[0],bbx_dim[1],bbx_dim[2]))).astype('int')
    add_count=torch.from_numpy(add_count)
    #label_1_mat = (np.zeros((bbx_dim[0],bbx_dim[1],bbx_dim[2]))).astype('uint8')
   # label_1_mat = torch.from_numpy(label_1_mat)
    #label_2_mat = (np.zeros((bbx_dim[0],bbx_dim[1],bbx_dim[2]))).astype('uint8')
    #label_2_mat = torch.from_numpy(label_2_mat)
    #label_3_mat = (np.zeros((bbx_dim[0],bbx_dim[1],bbx_dim[2]))).astype('uint8')
    #label_3_mat = torch.from_numpy(label_3_mat)
        ...
        for C in range(0, fold[1]):
            ...
            for H in range(0, fold[2]):
                ...
                if h_e >= bbx_dim[2]:
                    ...
                    h_e = h_s + grid_D
                # histogram for voting
                idx_0 = (grid_list[p_count])
                #idx_0=torch.from_numpy(idx_0)
                # idx_0 = (np.asarray(idx_0)).astype('int')
    ...
    label_mat = label_0_mat/add_count
    # print np.unique(label_mat)
